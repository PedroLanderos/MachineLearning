...
def sets_to_csv_row(d):
    ...
    for bf in BOOL_FIELDS:
        ...
        if not bset:
            ...
            tokens = ["True" if b else "False" for b in sorted(bset, key=lambda x: not x)]
            row[bf] = "|".join(tokens)
    return row
...
def append_or_merge(path, row_sets):
    ...
    if row_sets["nombre"] in idx:
        i = idx[row_sets["nombre"]]
        db[i] = merge_rows_sets(db[i], row_sets)
    else:
        db.append(row_sets)
...
def ask_all_questions(db_rows_sets):
    ...
    known_colors_txt = ", ".join(known_colors) if known_colors else "rojo, amarillo, verde, morado, naranja, rosa, marron, cafe, azul"
...
# -------- Modo aprendizaje (auto-merge con respuestas actuales) --------
def learning_mode_with_current_answers(csv_path, answers):
    ...
    # Construimos sets a partir de las respuestas actuales
    row_sets = {
        "nombre": nombre,
...
def play(csv_path):
    ...
    if guess:
        resp = to_lc(input(f"¿Es {guess}? (s/n) "))
...
def simulate_quality(csv_path):
    ...
    print(f"\nMETRICAS (multivalor, orden fijo) sobre {len(db)} frutas:")
    print(f"- Nº de preguntas promedio: {avg_q:.2f}")
    print(f"- % de acierto: {acc:.1f}%\n")

def main():
    parser = argparse.ArgumentParser(description="Akinator de frutas (multivalor + auto-merge + orden fijo)")
