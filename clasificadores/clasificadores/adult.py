# -*- coding: utf-8 -*-
"""
Evaluación de clasificadores en Adult Census Income:
...
Notas:
- El dataset Adult suele usar "?" para valores faltantes en variables categóricas.
  Aquí los convertimos a NaN para que el imputador los maneje.
- La columna objetivo típica es `income` (">50K" / "<=50K"). El detector intenta reconocerla.
- Se incluye un FIX para NuSVC: ajustamos 'nu' automáticamente a un valor factible
  según la fracción de la clase minoritaria en el split de entrenamiento.
"""
...
# 1) Carga del CSV
# ---------------------------------------------------------------------
CSV_PATH = "adult.csv"  # <-- ajusta si tu archivo tiene otro nombre/ruta
df = pd.read_csv(CSV_PATH)

# Reemplazar "?" por NaN (faltantes típicos en Adult)
df = df.replace("?", np.nan)

# Limpieza ligera de nombres de columnas (quitar espacios alrededor)
...
def detectar_objetivo(df):
    candidatos = ['income', 'Income', 'target', 'label', 'class', 'y']
    cols_map = {c.lower(): c for c in df.columns}
    for name in candidatos:
        if name.lower() in cols_map:
            return cols_map[name.lower()]
    # Heurística: columna con 2-3 clases y tokens típicos de income
    for c in df.columns:
        vals = pd.Series(df[c].dropna().unique())
        if vals.size in (2, 3):
            joined = " ".join(map(str, vals)).lower()
            if any(k in joined for k in [">50k", "<=50k", "50k", "k"]):
                return c
    raise ValueError(
        "No pude detectar la columna objetivo automáticamente. "
        "Renombra tu columna objetivo a 'income' (o usa 'target','label','class','y')."
    )
...
# ---------------------------------------------------------------------
def plot_crudo_pca_y_clases(X, y, outdir="figs_adult"):
    ...
    if len(num_cols) >= 2:
        ...
        # Estandarización ligera para PCA crudo
...
plot_crudo_pca_y_clases(X, y, outdir="figs_adult")
...
def plot_segmentado_pca(preprocessor, X_train, X_test, outdir="figs_adult"):
    os.makedirs(outdir, exist_ok=True)
...
plot_segmentado_pca(preprocessor, X_train, X_test, outdir="figs_adult")

# ---------------------------------------------------------------------
# 7) FIX NuSVC: elegir nu factible según clase minoritaria en train
# ---------------------------------------------------------------------
vc = pd.Series(y_train).value_counts(normalize=True)
minority_frac = float(vc.min())            # proporción de la clase minoritaria
NUSVC_NU = max(0.01, min(0.5, 0.9 * minority_frac))  # en (0.01, 0.5], margen 0.9
print(f"Usando NuSVC(nu={NUSVC_NU:.4f}) para garantizar factibilidad.")

# ---------------------------------------------------------------------
# 8) Definición de clasificadores (exactamente los solicitados)
# ---------------------------------------------------------------------
modelos = {
    "KNeighborsClassifier": KNeighborsClassifier(),
    "SVC": SVC(probability=False, random_state=RANDOM_STATE),
    "LinearSVC": LinearSVC(random_state=RANDOM_STATE),
    "NuSVC": NuSVC(nu=NUSVC_NU, probability=False, random_state=RANDOM_STATE),
...
# ---------------------------------------------------------------------
# 9) Entrenamiento, métricas y matrices de confusión
# ---------------------------------------------------------------------
def evaluar_modelos(modelos, preprocessor, X_train, X_test, y_train, y_test, outdir="figs_adult"):
    os.makedirs(outdir, exist_ok=True)
    registros = []
...
resultados = evaluar_modelos(modelos, preprocessor, X_train, X_test, y_train, y_test, outdir="figs_adult")

print("\n=== Resumen de métricas (ordenado por F1 weighted) ===")
print(resultados.round(4))

# ---------------------------------------------------------------------
# 10) Gráficas de barras de métricas comparativas
# ---------------------------------------------------------------------
def plot_metric_bars(resultados, outdir="figs_adult"):
    os.makedirs(outdir, exist_ok=True)
...
plot_metric_bars(resultados, outdir="figs_adult")

# ---------------------------------------------------------------------
# 11) Notas útiles de salida
# ---------------------------------------------------------------------
print("\nArchivos generados en ./figs_adult :")
...
print("\nListo. ¡Comparación completada para Adult Census Income!")
